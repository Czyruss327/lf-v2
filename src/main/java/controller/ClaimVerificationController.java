...
public class ClaimVerificationController implements Initializable {
    ...
    private static final Set<String> ALLOWED_UPLOAD_EXTENSIONS = Set.of("png", "jpg", "jpeg", "gif", "webp");
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        installLengthLimit(courseSectionField, MAX_COURSE_SECTION_LENGTH);
    ...
    private String validateUpload(File file, List<File> selectedFiles) {
        ...
        if (!ALLOWED_UPLOAD_EXTENSIONS.contains(fileExtension(file.getName()))) {
            return file.getName() + " is not a supported image file.";
        }
    ...
    private boolean saveClaimSlip(Claim claim, String claimantName, String courseSection,
        ...
        if (!confirmPdfGeneration("Generate Claim Slip",
                "Generate a claim slip for this approved claim?")) {
            return false;
        }
        ...
        chooser.setInitialFileName(buildClaimSlipFileName(claimantName, item.getName(), releasedDate, releaseTime));
        Stage stage = (Stage) claimNameField.getScene().getWindow();
    ...
    private void navigateToItemDetails() {
        ...
        try {
            ...
            Stage stage = (Stage) claimNameField.getScene().getWindow();
            SceneUtil.setScene(stage, root);
            stage.setTitle("Item Details - PUPSRC Lost and Found");
        } catch (IOException e) {
            e.printStackTrace();
            navigateBack();
        }
